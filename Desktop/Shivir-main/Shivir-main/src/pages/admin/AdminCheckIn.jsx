...
function StudentProfileModal({ student, onClose, onCheckIn, lastCheckedId }) {
      ...
      <div className="bg-white w-full sm:max-w-md rounded-t-3xl sm:rounded-2xl shadow-2xl fade-in overflow-hidden">
        ...
        <div className={`px-6 pt-6 pb-4 ${isIn ? 'bg-green-50' : 'bg-forest-50'}`}>
          ...
          <div className="flex flex-col items-center">
            ...
            <h2 className="mt-3 text-xl font-bold text-gray-900">{student.name}</h2>
          ...
          <div className={`flex items-center justify-between p-3 rounded-xl border-2 ${student.kit_given ? 'bg-green-50 border-green-300' : 'bg-amber-50 border-amber-300'}`}>
            ...
            <button
              onClick={async () => {
                ...
                await supabase.from('students').update({ kit_given: newVal }).eq('id', student.id);
              }}
...
function CheckInApp({ onBack }) {
  ...
  const tabList = students.filter(s => {
    if (tab === 'Checked In') return s.checked_in;
    if (tab === 'Pending') return !s.checked_in;
    return true;
  });
      ...
      >
        ...
        >
          ...
          <div className="text-sm text-gray-600 mt-0.5 truncate">
            {student.batch}{student.gender ? ` • ${student.gender}` : ''}{student.age ? ` • Age ${student.age}` : ''}
          </div>
          {student.parent_name && (
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-forest-700 text-white px-4 py-4 flex items-center gap-3">
        ...
        <div>
          <h1 className="font-bold text-lg leading-tight">Check-In Station</h1>
          <p className="text-xs text-forest-300">Wristband Distribution</p>
        </div>
...
export default function AdminCheckIn() {
  ...
  if (!unlocked) return <PasswordGate onUnlock={() => setUnlocked(true)} onBack={goBack} />;
  return <CheckInApp onBack={goBack} />;
}
